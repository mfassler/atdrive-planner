...
class WebRTCs:
    def __init__(self):
        ...
        self._nav_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._nav_sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEPORT, 1)
        self._nav_sock.bind(("0.0.0.0", 27204))
        self._nav_sock.setblocking(0)

        self._connection_counter = 1


    async def check_nav_packets(self):
        while True:
            # Empty out the UDP recv buffer and get only the final packet
            pkt = None
            addr = None
            while True:
                try:
                    pkt, addr = self._nav_sock.recvfrom(256)
                except BlockingIOError as bioe:
                    break

            if pkt is not None:
                #print('got packet:', len(pkt))
                lat, lon, hdg, spd = struct.unpack('!dddd', pkt)
                msg = json.dumps({'lat': lat, 'lon': lon, 'hdg': hdg, 'spd': spd})
                self.send_to_all(msg, dumbHack=True)
